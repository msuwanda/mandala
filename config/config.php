<?php
include_once $_SERVER['DOCUMENT_ROOT'].'/conf/setDB03.php';
//error_reporting(0) ;
class Pages extends DbConfig
{
    ...
    {
    $que 	= "SELECT app_exe,app_nama FROM tm_aplikasi WHERE app_id = '".$this->_parameter_pages."'";
		...
		if($count == 0){
    		return array("app_nama" => "null","app_exe" => "views/404") ;
    	}
    	else
    	{
    ...
    public function getContet() { return include $this->_app_exe ; }
		public function getScript() { return $this->_app_js ; }
    public function getMenu()
	{
		$que 	= "SELECT * FROM tm_aplikasi WHERE app_menu = '1' ORDER BY app_id ";
		$sth 	= $this->connection->prepare($que);
		$sth->execute();
